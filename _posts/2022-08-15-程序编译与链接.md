...
>[dl-resolve浅析](https://xz.aliyun.com/t/6364)
...
> glibc-2.22/sysdeps/i386/dl-trampoline.S用汇编实现

- 位于loader中，用于解析外部函数符号的函数
- 解析完成后会直接执行解析的函数
- link_map：一个链表，包含所有加载的共享库信息
- 在0xf7fee00b处调用_dl_fixup
...
> glibc-2.22/elf/dl-runtime.c实现

`_dl_fixup(struct link_map *l,ElfW(Word) reloc_arg)`

1. 通过参数reloc_arg计算.rel.plt对应的Elf32_Rel结构体，这里的JMPREL即.rel.plt，reloc_offset=reloc_arg
...
2. 通过reloc->r_info找到.dynsym中对应的Elf32_Sym结构体
	...
	`assert (ELFW(R_TYPE)(reloc->r_info)) == E;F_MACHINE_JMP_SLOT`
3. 通过sym->st_name+dynstr找到符号表字符串，通过_dl_lookup_symbol寻找libc基地址并返回给result

	`result = _dl_lookup_symbol_x(strtab+sym->st_name,l,&sym,l->l_scope,version,ELF_RTYPE_CLASS_PLT,flags,NULL)`

	value为libc基址加上要解析函数的偏移地址即实际地址
...
![Window shadow](/assets/img/2022-08/2022-08-15-%E7%A8%8B%E5%BA%8F%E7%BC%96%E8%AF%91%E4%B8%8E%E9%93%BE%E6%8E%A5/dl_fixup.drawio.svg){: .shadow}
_dl_fixup调用过程_
